...
export default function websocket(websocket: HostipWebSocket, request: IncomingMessage) {
  ...
  websocket.on("message", (text: string) => {
    ...
    const dynamicallyHandledMessageTypes = ["forwardedResponse", "WebSocketClientMessage", "WebSocketCloseMessage"];
...
function socketOverSocket(websocket: HostipWebSocket, request: IncomingMessage) {
